...
export default function LoadingScreen({ onComplete }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#030303] text-white font-mono overflow-hidden">
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at center, #0a0a0f 0%, #030303 100%)' }} />
      <div className="relative w-full max-w-md px-6 space-y-8">
        <div className="space-y-3">
          {bootLines.map((line, i) => (
            <div key={i} className={`flex justify-between text-sm transition-all duration-500 ${i <= index ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
        ...
        <div className="space-y-2">
          ...
          <div className="h-1 w-full bg-zinc-800 rounded-full overflow-hidden">
            <div className="h-full transition-all duration-300 ease-out" style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #7C3AED, #C026D3, #2DD4BF)' }} />
          </div>
        </div>
        {isLaunching && (
          <div className="text-center pt-8 animate-bounce">
            <h2 className="text-3xl font-bold bg-gradient-to-r from-violet-500 via-fuchsia-500 to-teal-400 bg-clip-text text-transparent">LAUNCHING ✦</h2>
          </div>
        )}
      </div>
    </div>
